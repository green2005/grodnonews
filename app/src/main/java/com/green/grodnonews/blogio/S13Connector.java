...
public class S13Connector {
    private static S13Connector sBlogConnector;
    ...
    public static S13Connector getBlogConnector() {
        if (sBlogConnector == null) {
            sBlogConnector = new S13Connector();
        }
    ...
    public InputStream getInputStream(String url, String charSet) throws IOException {
        ...
        try {
            ...
            response = mHttpClient.execute(rq);
        } finally {
            mIsBusy.set(false);
        }
        if (response == null) {
    ...
    public void loginAsync(final String login, final String pwd, final RequestListener listener) {
        final Handler handler = new Handler();
        new Thread(new Runnable() {
            @Override
            public void run() {
    ...
    class LoggingInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            Log.d("OkHttp", String.format("Sending body %s on %s%n%s",
    ...
    public boolean doLogin(String login, String pwd) throws Exception {
        ...
        try {
            ...
            Boolean b = page.toLowerCase().contains("logout");
            mIsLoggedIn.set(b);
            return b;
        } finally {
            mIsBusy.set(false);
